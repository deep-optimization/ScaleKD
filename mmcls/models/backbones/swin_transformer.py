...
class SwinTransformer(BaseBackbone):
    ...
    def forward_specific_stage(self, x, stage, down_sample=True):
        ...
        if stage == 2:
            if down_sample:
                hw_shape=(56,56)
                x, hw_shape = self.stages[-4].downsample(x, hw_shape)
            else:
                hw_shape=(28,28)

            x, hw_shape = self.stages[-3](
                x, hw_shape, do_downsample=self.out_after_downsample)

        if stage == 3:
            if down_sample:
                hw_shape=(28,28)
                x, hw_shape = self.stages[-3].downsample(x, hw_shape)
            else:
                hw_shape=(14,14)

            x, hw_shape = self.stages[-2](
                x, hw_shape, do_downsample=self.out_after_downsample)

        if stage == 4:
            if down_sample:
                hw_shape=(14,14) 
                x, hw_shape = self.stages[-2].downsample(x, hw_shape)
            else:
                hw_shape=(7,7)

            x, hw_shape = self.stages[-1](
                x, hw_shape, do_downsample=self.out_after_downsample)
    ...
    def forward_from_stage(self, x, stage=4):
        if stage==2:
            x = self.forward_specific_stage(x, 2)
            x = self.forward_specific_stage(x, 3)
            x = self.forward_specific_stage(x, 4)
        elif stage==3:
            x = self.forward_specific_stage(x, 3)
            x = self.forward_specific_stage(x, 4)
        else:
